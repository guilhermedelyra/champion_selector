...
def retrieve_sup_result(args=sys.argv[1:]):   
    ...
    for arg in args:
        ...
        if len(champ) >= 3:
            if "eadc" in arg:
                champs = return_champs(champ, "adcsupport", '/ADC?', champs, div, enemy=1)
            if "esup" in arg:
                champs = return_champs(champ, "matchups", '/Support?', champs, div, enemy=1)
            if "madc" in arg:
                champs = return_champs(champ, "synergy", '/ADC?', champs, div)
