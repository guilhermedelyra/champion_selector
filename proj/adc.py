...
def retrieve_adc_result(args=sys.argv[1:]):   
    ...
    for arg in args:
        ...
        if len(champ) >= 3:
            if "eadc" in arg:
               champs = return_champs(champ, "matchups", '/ADC?', champs, div, enemy=1)
            if "esup" in arg:
               champs = return_champs(champ, "adcsupport", '/Support?', champs, div, enemy=1)
            if "msup" in arg:
               champs = return_champs(champ, "synergy", '/Support?', champs, div)
